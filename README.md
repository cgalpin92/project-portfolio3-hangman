...
- __Play Game__
    ...
    - A list of dashes are presented to the player with a statement telling them that this is their hidden word.
    ...
    - A statement is printed to confirm the players input.
        - If correct:
...
## Data Model
 ...
 - I have used some global variabled which will be modified as the game is played and that can be accessed by multiple functions.
...
- __Fixed Bugs__
    ...
    - Error of hidden word not retaining previously correct guesses.
        - Found that when running through the game, the hidden word was not retaining previously correct guesses. For example:
            ...
            - when the player guesses another letter, e, the terminal wil print - - e - -
...
- Understanding and Code for updating the hidden word taken from Stack Overflow - https://stackoverflow.com/questions/67609834/how-to-replace-the-specified-dash-with-the-letter
    - This was later removed as found a better way to update the hidden word with the correct letter/word. This was so I could fix the bug where the hidden word was continuously updated with each correct letter/word. This new code allows me to first store each individual letter in a positions variable which which allows me to iterate through the letters. The hidden word is then converted into a list so that it can be modified with each new correct letter. The list is then turned back into a string which is then printed to the terminal. Found this in existing hangman game, and Stack Overflow pages which further helped my understanding:
    - Exisiting hangman game - https://github.com/paulio11/P3-Hangman-Python-Terminal-Game/blob/main/run.py#L510
    ...
    - The existing hangman game also gave me the idea for how to write the code to iterate through the hangman pictures. As a string cannot be modified, I would need to store an integer within another varibale, the incorrect guess would update this integer which will specify the next hangman picture in the sequence to be called.
